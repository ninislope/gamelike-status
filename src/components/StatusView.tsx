...
export const StatusView = injectSafe("ui")(observer<React.SFC<{character: Character; ui: UI}>>(({character, ui}) => {
    if (!character.periods.length) {
        return <div className={classNames(style.wrapper, style.addMessage)}><div>時期を追加して下さい</div></div>;
    }
    const period = character.period(ui.period);
    if (!period) {
        return <div className={classNames(style.wrapper, style.addMessage)}><div>時期を選択して下さい</div></div>;
    }
        ...
        <div key="PageView">
            {
                ...
                <div className={style.addMessage}>ページを追加して下さい</div>
            }
        </div>
